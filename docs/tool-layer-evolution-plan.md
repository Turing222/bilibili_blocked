...
start-chrome.ps1            启动层  --remote-debugging-address=127.0.0.1 + --remote-debugging-port=9223 + 独立 profile
  └─ 手动登录一次            登录态存 repo 外,幂等复用
       │
       ▼
inspect-comments.mjs         探针层  手搓裸 CDP:WebSocket + Runtime/Page/Network 域
       │                            输出固定结构 JSON,给 agent 读 / 做回归 oracle
       ▼
...
1. **探针层 `inspect-comments.mjs` 是手搓的裸 CDP 客户端**（`CdpClient` 类、`/json/list` 发现、`Runtime.evaluate` 透传），零第三方依赖，定位是「给 agent 读页面状态的确定性接口 + 回归 oracle」。
2. **流程层 `comment-timing-smoke.mjs` / `video-card-timing-smoke.mjs` 已经是 Playwright**（`chromium.connectOverCDP`），但部分用法没吃上 Playwright 红利——`queryAllDeep` 抄了 5 份、固定坐标 + `sleep` 轮询评论、用 `evaluate` 做本该用 Locator 做的点击/可见性判断。
...
用 `chrome-devtools-mcp` 的 **full mode**（不加 `--slim`）作为 agent 交互入口。`inspect-comments.mjs` 暂保留为确定性 oracle（见 §7 阶段 3 迁移其底层，不归档）。
...
**Codex Windows（`.codex/config.toml`）**——不能直接套 Cursor JSON，需经 `cmd /c npx` 启动并放宽超时：
...
- **锁文件 / 浏览器 lease**：MCP 探索与 Playwright smoke 使用同一个 `artifacts/locks/browser-9223.lock`。获取锁必须是原子操作（例如创建锁目录，或用独占创建文件），不能先检查再写入。结束时释放锁；发现已有锁时必须停止操作并提示当前 owner。
...
> 【红线】不能用「agent 主观判断结果一致」当验收。要拆成**工具层验收**（MCP/CLI 与原探针结构化结果一致）和 **agent 层验收**（agent 能正确选页/调工具/解释结果）两层，失败时才能定位是哪层的问题。

- [ ] 配 `chrome-devtools-mcp`（锁版本，§4.2.1），`npm run pw:chrome` 启动并手动登录。
- [ ] 确认/补齐启动层安全参数：`--remote-debugging-address=127.0.0.1` + `--remote-debugging-port=9223`，并验证 9223 只监听回环地址。
- [ ] **工具层验收（绕开 agent）**：用实验性 CLI 直接出 JSON，与 `inspect-comments.mjs` 的 oracle 做差分：

```bash
chrome-devtools list_pages --output-format=json
chrome-devtools evaluate_script --function "() => ({ href: location.href, title: document.title })" --output-format=json
```

  同一标签、同一页面状态下：`inspect-comments.mjs → oracle.json`，`MCP/CLI → mcp.json`，经 `normalize-results.mjs` 归一化后 diff。需归一化的字段：时间戳、requestId、元素顺序、动态计数、临时 URL 参数、页面加载时间。
- [ ] **agent 层验收**：让 agent 用 MCP 跑通 `list_pages → select_page(bringToFront=true) → navigate_page → evaluate_script(读 bili-comments Shadow DOM + __data) → list_network_requests`，并正确解释结果。
- [ ] 验收：工具层 diff 通过 + agent 层能正确选页/调工具/解释结果。

### 阶段 2：流程层 Playwright 内部重构

- [ ] 先跑 `pw:comment-timing` / `pw:video-card-timing`，存 `result.json` / `events.jsonl` 为基线。
- [ ] 按 §4.3.2 清单逐项改：抽 `queryAllDeep` / `readDataMessage` / `getCommentText` 进 `scripts/lib/bilibili-dom.js`；抽 `createGmRuntimeStub()` 进 userscript runtime helper；可见元素换 `Locator`（不用 `>>>`）；`scrollUntil` 替换固定滚动+sleep；`waitForResponse` 替换单步网络断言；扩充现有 `tests/automation-harness.test.js` 或按模块新增对应单测。
- [ ] **注入时序验证任务**（§4.4）：对比真实油猴默认 `@run-at` 与 harness 注入时机；必要时显式声明 `@run-at`，再定方案 A/B。
- [ ] 加浏览器锁（§5），并让 MCP 探索与 Playwright smoke 都遵守同一个 browser lease。
- [ ] 验收：smoke 仍 `ok: true`，`result.json` 顶层结构不变，`events.jsonl` 关键 `kind` 不变。
...
- [ ] 把 `inspect-comments.mjs` 底层从手搓 `WebSocket` + `CdpClient` 改成 **Playwright `connectOverCDP` + `newCDPSession` / `evaluate`**，调用与流程层共享的 DOM extractor（阶段 2 抽出的 `bilibili-dom.js`）。
- [ ] **保留 `inspect-comments` 命令名与 JSON 输出契约不变**。
- [ ] 验收：迁移后 `inspect-comments.mjs` 输出与迁移前结构一致，且仍能作阶段 1 的 oracle。
...
`inspect-comments` 命令与 JSON 输出契约长期保留，作为 MCP 的 oracle / 对照组 / 逃生通道，不因 MCP 稳定而废弃。
...
| 流程层重构引入行为差异 | smoke 误判 | 阶段 2 基线 `result.json` / `events.jsonl` diff，逐项改逐项验 |
...
- **阶段 1**：工具层 `oracle.json` vs `mcp.json` 归一化 diff 通过；agent 层能正确选页/调工具/解释结果。
- **阶段 2**：`pw:comment-timing` / `pw:video-card-timing` 重构后 `ok: true`，`result.json` 顶层结构不变，`events.jsonl` 关键 `kind` 不变；注入时序验证有结论。
- **阶段 3**：`inspect-comments.mjs` 迁移底层后 JSON 输出结构与迁移前一致，仍能作 oracle。
...
| 路径 | 本方案中的角色 |
|---|---|
| `tools/bilibili-browser/start-chrome.ps1` | 启动层，入口和行为保持现状，只补安全参数与监听校验 |
| `tools/bilibili-browser/inspect-comments.mjs` | 探针层，阶段 1 作 oracle，阶段 3 迁底层（不归档） |
| `scripts/comment-timing-smoke.mjs` | 流程层，阶段 2 重构 |
| `scripts/video-card-timing-smoke.mjs` | 流程层，阶段 2 重构 |
| `scripts/lib/harness.js` | 现有公共 Node 骨架；保留参数、产物、recorder、页面选择等非 DOM 契约 |
| `scripts/lib/browser-harness.js`（可选新建） | 阶段 2 接收 `scrollUntil`、浏览器锁等 Playwright Page 相关 helper |
| `scripts/lib/bilibili-dom.js`（新建） | 阶段 2/3 共享的 DOM extractor，接收 `queryAllDeep` / `readDataMessage` / `readTextDeep` / `getCommentText` |
| `scripts/lib/userscript-runtime.js`（可选新建） | 阶段 2 接收 `createGmRuntimeStub()` 与 userscript 注入时序相关 helper |
| `tests/automation-harness.test.js` | 阶段 2 扩充现有单测；必要时按新 helper 拆分新增测试文件 |
| `.codex/config.toml` / Cursor MCP 配置 | 阶段 1 配 `chrome-devtools-mcp` |
