...
**实施状态（2026-06-29）**：阶段 1–4 的代码主体已在最近提交中落地（commits `fdace96` → `2998fcb`）；本文为后续收尾状态说明。日常命令见 [`tools/bilibili-browser/README.md`](../tools/bilibili-browser/README.md) Phase 1 节与下方 §7 收尾表。
...
**仍可选 / 未强制验收**：阶段 1 **agent 层** MCP 手工走查（§7）；阶段 2 **`waitForResponse`** 与更多 **Locator** 替换（§4.3.2 余量）；**`ci:deterministic-smoke`**（§4.3.5，后续另立方案）。
...
2. **流程层** smoke 已抽公共 lib、`scrollUntil`、browser lease；部分 Locator / `waitForResponse` 优化仍可作为后续增量。
...
- [x] 配 `chrome-devtools-mcp`（锁版本，§4.2.1），`npm run pw:chrome` 启动并手动登录。→ [`.cursor/mcp.json`](../.cursor/mcp.json)、[`.codex/config.toml`](../.codex/config.toml)
...
- [ ] **agent 层验收**（需 Reload MCP 后人工走查）：让 agent 用 MCP 跑通 `list_pages → select_page(bringToFront=true) → navigate_page → evaluate_script(读 bili-comments Shadow DOM + __data) → list_network_requests`，并正确解释结果。
- [x] 工具层 diff 已通过（2026-06-29，`diffs: []`）。agent 层见上一条。
...
- [ ] `waitForResponse` 替换单步网络断言；全面 Locator 化可见元素（§4.3.2 余量，非阻塞）。
...
- **阶段 1**：工具层 `oracle.json` vs `mcp.json` 归一化 diff 通过 ✅（2026-06-29）；agent 层能正确选页/调工具/解释结果 ⏳（待 MCP Reload 后走查）。
- **阶段 2**：`pw:comment-timing` / `pw:video-card-timing` 重构后 `ok: true` ✅；注入时序结论见 [`injection-timing-verification.md`](./injection-timing-verification.md) ✅。
...
| `scripts/lib/browser-harness.js` | `scrollUntil`、browser lease、owner-token 释放校验 |
...
| `tests/browser-harness.test.js` | browser lease / scrollUntil 单测 |
| `tests/normalize-results.test.js` | oracle 归一化 diff 单测 |
| `docs/injection-timing-verification.md` | §4.4 注入时序结论 |
| `.codex/config.toml` / `.cursor/mcp.json` | 阶段 1 `chrome-devtools-mcp@1.4.0` |
