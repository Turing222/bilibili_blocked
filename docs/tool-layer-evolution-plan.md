...
| CI deterministic smoke | `npm run ci:deterministic-smoke` |

**仍可选 / 未强制验收**：阶段 2 进一步减少页面侧状态读取脚本（不强制消灭合理 `evaluate`）；`ci:deterministic-smoke` 后续可继续扩展更多 API / UI fixture 覆盖。
...
| `ci:deterministic-smoke`（v1 已落地） | Playwright 自启浏览器 + 内存 B 站 fixture + mock API + 隔离 context | 正式 CI gate 的第一层无登录烟测 |

若目前只需本机验证，至少把文档/脚本里的「CI gate」改称「本地回归 gate」，避免对可重复性的描述高于实际能力。

v1 覆盖面：不连接 `127.0.0.1:9223`，不复用登录 profile；通过本地 fixture 模拟视频卡片、`bili-comments` open shadow DOM、评论 `__data`，并断言初始视频遮罩、Mutation 新卡片 fast path、评论遮罩删除、浮窗关闭清理。已接入 GitHub Actions [`ci.yml`](../.github/workflows/ci.yml)，每次 push / PR 先跑 lint + check，再跑 deterministic smoke。后续扩展可以再补更多 API fixture、设置面板交互或测试账号 `storageState` 的独立 nightly gate。
...
- [x] `ci:deterministic-smoke` v1：[`ci-deterministic-smoke.mjs`](../scripts/ci-deterministic-smoke.mjs) 自启浏览器、加载本地 B 站 fixture、mock API、注入 dist userscript，并验证视频卡片遮罩 / 新卡片 fast path / 评论遮罩删除 / 浮窗关闭清理；[`ci.yml`](../.github/workflows/ci.yml) 已把该 smoke 接入 push / PR gate。
...
- **阶段 2**：`pw:comment-timing` / `pw:video-card-timing` 重构后 `ok: true` ✅；评论接口等待已接入 `waitForResponse` ✅（2026-06-29）；关键可见交互 Locator 化 ✅（2026-06-29）；`ci:deterministic-smoke` v1 ✅（2026-06-29）；注入时序结论见 [`injection-timing-verification.md`](./injection-timing-verification.md) ✅。
...
| `scripts/ci-deterministic-smoke.mjs` | CI deterministic smoke v1，自启浏览器 + fixture + mock API |
| `.github/workflows/ci.yml` | GitHub Actions gate：lint / check / CI deterministic smoke |
